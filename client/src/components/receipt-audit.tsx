...
export default function ReceiptAudit() {
  ...
  const handleAudit = async (textToAudit?: string) => {
    ...
    try {
      // Endpoint rewrite connects to FastAPI /api/audit
      ...
      console.error(err);
      // Fallback direct backend call if proxy fails
      try {
        const directRes = await fetch("http://127.0.0.1:8000/api/audit", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ text }),
        });
        if (directRes.ok) {
          const directData: AuditResponse = await directRes.json();
          setAuditData(directData);
          setLoading(false);
          return;
        }
      } catch (directErr) {
      ...
      setError("Failed to connect to audit server. Ensure FastAPI backend is running.");
    } finally {
      setLoading(false);
    }
  };
          ...
          {auditData ? (
            ...
            >
              ...
              <div className="text-center border-b-2 border-dashed border-slate-400 pb-5 mb-6">
                ...
                <p className="text-[11px] text-slate-500">FastAPI Engine + Groq Pydantic v2 Audit</p>
              </div>
                  ...
                  >
                    <div className="flex justify-between items-start gap-2 mb-1">
                      <div className="relative font-bold text-sm text-slate-800 pr-2">
                        ...
                        {/* Framer Motion v12 Visual Strikethrough Slash Animation for Illegal Deductions */}
                        {item.is_illegal && (
                          <motion.div
                            initial={{ scaleX: 0 }}
                            animate={{ scaleX: 1 }}
                    ...
                    {/* Illegal Routine Maintenance Badge & Explanation */}
                    {item.is_illegal ? (
              ...
              {/* Bold Statutory Recovery Counter */}
              <motion.div
            ...
            <div className="h-full min-h-[420px] rounded-2xl border-2 border-dashed border-slate-800 bg-slate-900/50 p-8 flex flex-col items-center justify-center text-center text-slate-400">
              ...
              <p className="text-xs text-slate-500 max-w-xs mt-1">
                Paste your landlord notice on the left or click "Audit Sample Notice" to see live Framer Motion deduction strikethroughs & statutory recovery calculations.
              </p>
            </div>
          )}
        </AnimatePresence>
